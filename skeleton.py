...
class Game:
    ...
    def is_valid_move(self, coords: CoordPair) -> bool:
        ...
        if self.get(coords.dst) is not None and self.get(coords.dst).player == unit.player and self.get(
                coords.dst).health == 9:
            return False

        elif (self.get(coords.dst) is not None and self.get(coords.dst).player == unit.player and unit.repair_amount(
                self.get(coords.dst)) != 0):
            return True
    ...
    def suggest_move(self) -> CoordPair | None:
        ...
        if self.options.alpha_beta:
            ...
            (score, move) = self.minimax(self.options.max_depth, True, self.options.max_time)

        elapsed_seconds = self.getTimeRunning() if self.getTimeRunning() < self.options.max_time else self.options.max_time
